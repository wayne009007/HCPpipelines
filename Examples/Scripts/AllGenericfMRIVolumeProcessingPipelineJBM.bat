...
for Subject in $Subjlist ; do
  for fMRIName in $Tasklist ; do
    ...
    DwellTime="0.00072" #Echo Spacing or Dwelltime of fMRI image = 1/(BandwidthPerPixelPhaseEncode * # of phase encoding samples): DICOM field (0019,1028) = BandwidthPerPixelPhaseEncode, DICOM field (0051,100b) AcquisitionMatrixText first value (# of phase encoding samples) 
    ...
    MagnitudeInputName="${StudyFolder}/${Subject}/mag_raw_concat_23.nii.gz" #Expects 4D Magnitude volume with two 3D timepoints, set to NONE if using TOPUP
    PhaseInputName="${StudyFolder}/${Subject}/ph_MB_23.nii.gz" #Expects a 3D Phase volume, set to NONE if using TOPUP
