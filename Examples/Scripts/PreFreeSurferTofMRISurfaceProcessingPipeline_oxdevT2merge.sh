...
Usage() {
  ...
  echo "             [--finishwith=<use if you want to finish the pipeline before the last step fMRISurface; set to the one of the following (this will be the last run step):>"
  echo "                         <prefreesurfer> <freesurfer> <postfreesurfer> <fMRIVolume>] "
...
if  [[  $finishwith == "prefreesurfer" ]] ; then
    ...
    echo "Finish the pipeline at the end, at the fMRISurface part"
fi
